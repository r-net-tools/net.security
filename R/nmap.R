...
ParseNMAP <- function(file.input, file.output.header = "") {
  ...
  # Get scan information for each host
  i = 1
  dns.info  <- data.frame()
  ...
  while (i <= XML::xmlSize(nmap.info)) {
    if (XML::xmlName(nmap.info[[i]]) == "host") {
      ...
      if ("hostnames" %in% names(XML::xmlChildren(nmap.info[[i]])) & XML::xmlValue(XML::xmlChildren(nmap.info[[i]])$hostnames) !=
        "\n") {
        ...
        while (j <= length(hostnames)) {
          ...
          host.hostname <- as.data.frame(t(XML::xmlAttrs(hostnames[[j]])))
      ...
      if ("ports" %in% names(XML::xmlChildren(nmap.info[[i]]))) {
        ...
        while (j <= length(ports)) {
          if (XML::xmlName(ports[[j]]) == "port") {
            ...
            nmap.data <- plyr::rbind.fill(nmap.data, cbind(host.address, host.status, host.port))
          }
          j = j + 1
        }
      } else {
        nmap.data <- plyr::rbind.fill(nmap.data, cbind(host.address, host.status))
      }
    }
    i = i + 1
  }
